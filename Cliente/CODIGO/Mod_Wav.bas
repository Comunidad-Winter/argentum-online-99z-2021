...
Dim LastNavegando As Long 'GS

Function LoadWavetoDSBuffer(DS As DirectSound, DSB As DirectSoundBuffer, sFile As String) As Boolean


    If sFile <> SND_NAVEGANDO Or _
     ((DirectX.TickCount - LastNavegando) > 6000) Then 'GS
        If sFile = SND_NAVEGANDO Then
            LastNavegando = DirectX.TickCount
        End If
    Else
        Exit Function
    End If 'GS
